...
from encoding.utils.utils import dfboxes, Encoder, PostProcess
...
class _SSD(nn.Module):
	def __init__(self, backbone=None, num_classes=21):
		...
		self.num_defaults = [4,4,4]
		location_extractors = []  # 定位预测器,使用3*3卷积来预测定位
		confidence_extractors = []  # 置信度预测器, 同样使用3*3卷积来预测
		self.feature_out_channels = [64,64,512]
		# out_channels = [1024, 512, 512, 256, 256, 256] for resnet50
		for nd, oc in zip(self.num_defaults, self.feature_out_channels):
		...
		default_box = dfboxes()  # 通过该函数生成默认的8732个default box
	...
	def forward(self, feature_maps, targets=None):
		...
		# locs:(batch_size, 4, 8732), confs:(batch_size, 21, 8732)
		locs, confs = self.bbox_view(feature_maps, self.loc, self.conf)
